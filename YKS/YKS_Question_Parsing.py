...
for sayfa_no in range(len(doc)):
    sayfa = doc[sayfa_no]
    bloklar = sayfa.get_text("blocks") # Font aramayacağımız için güvenilir "blocks" yapısına döndük
    ...
    for blok in bloklar:
        ...
        if blok_tipi == 0:  # Sadece metin blokları
            ...
            if match:
                ...
                # KRİTİK FİLTRE 1: ÖSYM'nin tuzak başlıklarını atla!
                if "Bu testte" in metin or "Cevaplarınızı" in metin:
                    continue

                # KRİTİK FİLTRE 2: "1. ve 4." gibi küçük harfle başlayan şık tuzaklarını atla!
                # (Bold kontrolü yerine en sağlam ve hatasız yöntem budur)
                if re.search(r'^\s*\d+\.\s+[a-z]', metin):
                    continue

                soru_no = match.group(1)

                # DİKKAT: ÖSYM marjinleri için x0 < 100 yerine x0 < 120 kullanıldı.
                if x0 < orta_cizgi and x0 < 120:
                    sol_sutun_sorular.append({"no": soru_no, "y0": y0})
                elif x0 > orta_cizgi and x0 < orta_cizgi + 120:
